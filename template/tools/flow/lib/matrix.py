...
class steps:
    ...
    def configure_cmake(config: dict):
        ...
        APP_SANITIZE = "ON" if config.get("sanitizer") else "OFF"
        APP_CUTDOWN_OS = "ON" if runner.CUTDOWN_OS else "OFF"
        runner.call(
            "cmake",
            "--preset",
            f"{config['preset']}-{config['build_generator']}",
            f"-D@APP_PREFIX@_SANITIZE:BOOL={APP_SANITIZE}",
            f"-D@APP_PREFIX@_CUTDOWN_OS:BOOL={APP_CUTDOWN_OS}",
        )
